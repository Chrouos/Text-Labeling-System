...
# 定義來源和目標資料夾
source_file_path = './other/re_random_100.json'
            ...
            for key, value in processed_row.items():
                if key == "name" and value in df.columns:
                    ...
                    if pd.isna(cell_value):
                        data['processed'][processed_index]['regular_expression_match'] = ""
                    else:
                        if isinstance(cell_value, pd.Int64Dtype().type):  # 檢查是否為 int64
                            data['processed'][processed_index]['regular_expression_match'] = int(cell_value)  # 轉換為 int
                        else:
                            data['processed'][processed_index]['regular_expression_match'] = cell_value
